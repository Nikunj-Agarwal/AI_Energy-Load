...
os.makedirs(os.path.join(MODELS_DIR, 'gru'), exist_ok=True)
...
def build_gru_model(input_shape, gru_units=64, dropout_rate=0.2):
    """Build GRU model for energy load prediction"""
    model = Sequential([
        # First GRU layer with return sequences for stacking
        GRU(gru_units, activation='tanh', return_sequences=True, 
            input_shape=input_shape, recurrent_dropout=0.0),
        BatchNormalization(),
        Dropout(dropout_rate),
        
        # Second GRU layer
        GRU(gru_units//2, activation='tanh', recurrent_dropout=0.0),
        BatchNormalization(),
        Dropout(dropout_rate),
...
def train_model(X_train_seq, y_train_seq, X_val_seq, y_val_seq, model_name, epochs=100, batch_size=32):
    ...
    checkpoint_dir = os.path.join(MODELS_DIR, 'gru', 'checkpoints')
    ...
    # Save final model
    model_path = os.path.join(MODELS_DIR, 'gru', f'{model_name}.keras')
...
def evaluate_model(model, X_test_seq, y_test_seq, test_idx, model_name):
    ...
    eval_dir = os.path.join(EVAL_DIR, f'gru_{model_name}')
    os.makedirs(eval_dir, exist_ok=True)
...
def create_evaluation_plots(predictions_df, eval_dir, model_name):
    ...
    plt.title(f'Actual vs Predicted Energy Load - GRU {model_name}')
    plt.xlabel('Date')
    ...
    plt.title(f'Error Distribution - GRU {model_name}')
    plt.xlabel('Error')
    ...
    plt.title(f'Error over Time - GRU {model_name}')
    plt.xlabel('Date')
    plt.ylabel('Error')
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    ...
    plt.title(f'Actual vs Predicted Scatter - GRU {model_name}')
...
def main():
    ...
    print("==== Energy Load GRU Model Training ====")
    ...
    # Define sequence length for GRU (6 hours of 15-min intervals)
    seq_length = 24
    ...
    for strategy in strategies:
        try:
            ...
            print(f"\n{strategy} GRU model training and evaluation completed!\n")
    ...
    print("\nAll GRU model training and evaluation completed!")
