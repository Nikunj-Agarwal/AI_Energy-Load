...
def build_lstm_model(input_shape, lstm_units=64, dropout_rate=0.2):
    """Build LSTM model for energy load prediction with GPU optimization"""
    
    # Enable CuDNN acceleration if available
    if tf.test.is_built_with_cuda():
        lstm_layer = tf.keras.layers.LSTM
    else:
        # Fall back to standard LSTM if CUDA not available
        lstm_layer = tf.keras.layers.LSTM
    
    model = Sequential([
        # First LSTM layer with return sequences for stacking
        lstm_layer(lstm_units, activation='relu', return_sequences=True, 
                  input_shape=input_shape),
        BatchNormalization(),
        Dropout(dropout_rate),
        
        # Second LSTM layer
        lstm_layer(lstm_units//2, activation='relu'),
        BatchNormalization(),
        Dropout(dropout_rate),
...
def train_model(X_train_seq, y_train_seq, X_val_seq, y_val_seq, model_name, epochs=100, batch_size=32):
    ...
    callbacks = [
        ...
        ModelCheckpoint(
            filepath=os.path.join(checkpoint_dir, f'{model_name}_best.keras'),  # Changed .h5 to .keras
            monitor='val_loss',
            save_best_only=True,
            verbose=1
        ),
    ...
    # Save final model
    model_path = os.path.join(MODELS_DIR, f'{model_name}.keras')  # Changed .h5 to .keras
    model.save(model_path)
    print(f"Model saved to {model_path}")
    
    return model, history

def evaluate_model(model, X_test_seq, y_test_seq, X_test, y_test, test_idx, model_name):
...
def create_evaluation_plots(predictions_df, eval_dir, model_name):
    ...
    plt.title(f'Actual vs Predicted Energy Load - {model_name}')
    plt.xlabel('Date')
    ...
    plt.title(f'Error over Time - {model_name}')
    plt.xlabel('Date')
    plt.ylabel('Error')
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
...
def main():
    ...
    print("==== Energy Load LSTM Model Training ====")
    ...
    # Define sequence length for LSTM (6 hours of 15-min intervals)
    seq_length = 24
    ...
    for strategy in strategies:
        try:
            ...
            model_name = f"lstm_energy_{strategy}_{timestamp}"
            ...
            print(f"\n{strategy} model training and evaluation completed!\n")
